...
// server's root
char* root = NULL;                                                              //pointer to string of root of server: the path

// file descriptor for sockets
int cfd = -1, sfd = -1;                                                         //like FILE* -- but use integers instead of pointers

// flag indicating whether control-c has been heard
bool signaled = false;                                                          //using cntlr+c
...
int main(int argc, char* argv[])
{
    ...
    // start server
    start(port, argv[optind]);                                                  //passing in port to start server

    // listen for SIGINT (aka control-c)                                        //using cntrl+c, the special signal, ends the program (hard coded)
    struct sigaction act;
    act.sa_handler = handler;                                                   //calling handler to accomplish this
    act.sa_flags = 0;
    ...
    while (true)                                                                //should be infinite, always checking for browser interaction
    {
        ...
        {
            // check for request
            if (request(&message, &length))                                     //takes what is in envelop and loads all of initial lines (like get) into varibale called message 
            {                                                                   //we are using a double pointer ** the address of the pointer. 
                // extract message's request-line                               //take address of pointer so we can go there and put a pointer there
                // http://www.w3.org/Protocols/rfc2616/rfc2616-sec5.html
                const char* haystack = message;                                 //no change with const
                const char* needle = strstr(haystack, "\r\n");                  //strstr searches for one string within another, we seach for end of line so we can read one line into memory
                if (needle == NULL)
                {
                    error(500);
                    continue;
                ...
                if (parse(line, abs_path, query))                               //parse extracts absolute path of the query.  
                {                                                               //need to interate over line, character by character, to end of string looking for query and absolute path 
                    // URL-decode absolute-path                                 //method (GET) 
                    char* p = urldecode(abs_path);                              //request-targer (what you parse)
                    if (p == NULL)                                              //HTTP-version (1.1)
                    {
                        error(500);
                        continue;
                    }
                    ...
                    path = malloc(strlen(root) + strlen(p) + 1);                //if user request /hello.html what file do they really man (root to server)
                    if (path == NULL)
                    {
                        error(500);
                        continue;
                    ...
                    // ensure path exists
                    if (access(path, F_OK) == -1)                               //if path doesn't exist, return not 200
                    {
                        error(404);
                        continue;
                    }
                    ...
                    struct stat sb;                                             //we need to change ULR to end in / if user didn't enter one
                    if (stat(path, &sb) == 0 && S_ISDIR(sb.st_mode))
                    {
                        ...
                        char* index = indexes(path);                            //we don't want to show contents of directory, but the contents of default file index.html or index.php
                        if (index != NULL)
                        {
                            free(path);
                            path = index;
                        ...
                        {
                            list(path);                                         //***just list the contents of that directory***
                            continue;
                        }
                    }

                    // look up MIME type for file at path
                    const char* type = lookup(path);                            //tell server if this is .gif or .jpeg or other
                    if (type == NULL)
                    {
                        error(501);
                        continue;
                    ...
                    if (strcasecmp("text/x-php", type) == 0)                    //transfer file from server to user
                    {
                        interpret(path, query);                                 //***interpret the file for the client***
                    }

                    // transfer file at path
                    else
                    {
                        transfer(path, type);                                   //***for other file types***
                    }
                }
            //else                                                              //in youtube video, not in my code
            //    {
            //    error(501)
            //    }
            }
...
void error(unsigned short code)
{
    ...
    if (phrase == NULL)
    {
        return;                                                                 //if there is no reason code
    }
 ...
 * Handles signals.
 */
void handler(int signal)                                                        //when hitting cntrl+c
{
    // control-c
    if (signal == SIGINT)
    {
...
char* indexes(const char* path)
{
    char* path_copy1 = malloc(strlen(path) + strlen("/index.html") + 1);
    strcpy(path_copy1, path);
    
    char* path_copy2 = malloc(strlen(path) + strlen("/index.html") + 1);
    strcpy(path_copy2, path);
    
    char final_php[100] = "index.php";                                      
    char final_html[100] = "index.html";
   
    strcat(path_copy1, final_php);                                              
    if (access(path_copy1, F_OK) == 0) 
        {
        return (path_copy1);
        }

    strcat(path_copy2, final_html);
    if (access(path_copy2, F_OK) == 0)                                      
        {
        return (path_copy2);
        }
 ...
 */
    bool load(FILE* file, BYTE** content, size_t* length)
    {
        int index = 0;
        int c; 
        //setting buffer pre-realloc
        char* buffer = malloc(1);              
        
        //before EOF, copy character by character
        while ((c = fgetc(file)) != EOF)                
            {
            //load characters into buffer
            buffer[index] = c;                          
            index++;
            //extends the buffer as needed
            buffer = realloc(buffer, sizeof(int) * index * sizeof(buffer));
            }
            
        //stores bytes in content
        *content = buffer;    
        //stores number of bytes in length
        *length = index;                                    
 ...
 */
const char* lookup(const char* path)                                            //http://cs50.stackexchange.com/questions/16983/i-cant-figure-out-how-to-fix-my-server-even-after-using-valgrind-and-gdb
{
    const char target = '.';
    char *returnValue = NULL;
    char *ret = strrchr(path, target);
    
    char temp[1000];                                                            //used to get lowercase
    strcpy(temp, ret);
    ...
    else if (strcmp(temp, ".php") == 0)
        returnValue = "text/x-php";
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    int absolute_end, query_begin, query_end;
    char* absolute_path = malloc(sizeof(LimitRequestLine + 1));
    char* query_string = malloc(sizeof(LimitRequestLine + 1));
    
            const char target = ' ';                                             //for http check, looks for last space
            char *version_check = strrchr(line, target);                         //holds everything following last space
                                    
    
            char query_target = '?';
            char* query_available = NULL;
            query_available = (strchr(line, query_target));                      //holds everything following first question mark
    
            char query_abs_targer = '/';
            char* query_abs = NULL; 
            query_abs = strchr(line, query_abs_targer);
    ...
    if (line[0] == 'G' && line[1] == 'E' && line[2] == 'T' && line[3] == ' ')
        ...
        for (int i = 0; i < strlen(line); i++)
            ...
            if (strcmp(version_check, " HTTP/1.1\r\n") != 0)                      //remember the \r\n!
                {
                error(505);
                return false; 
                }  
            
            if (query_available == NULL)
                {
                if (line[4] == '/' && line[i] == ' ')
                    {
                    absolute_end = i-1;
        ...
        if (query_available != NULL)
                {
                query_available = (strchr(line, query_target) + 1);    
                int n = (strlen(query_available) - strlen(version_check));
                strncpy(query, query_available, n);
                
                int y = (strlen(query_abs) - strlen(version_check) - n - 1);
                strncpy(abs_path, query_abs, y);
                }

        if (query_available == NULL)
            {
            for (int i = 0, q = 4; q <= absolute_end; i++, q++)                     //copy over appropriate parts for absolute path
                {
                absolute_path[i] = line[q];                                         //abs_path = address[4] through address[absolute_end];
                absolute_path[i+1] = '\0';
                }
        ...
        if (query_available == NULL)
            {
            for (int i = 0, q = query_begin; q <= query_end; i++, q++)              //copy over appropriate parts for query
                {
                query_string[i] = line[q];                                          //query = address[queary_begin] through address[query_end];
               }
