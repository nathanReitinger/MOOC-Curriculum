...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		List<Integer>one_hop = new ArrayList<Integer>();
		List<Integer> two_hop = new ArrayList<Integer>(); 
		for (int i = 0; i < getNumVertices(); i++) {
			for (int q = 0; q < adjMatrix[v][i]; q++) {
				//instead of grabbing just one hop, get two hop as well
				two_hop.addAll(getNeighbors(i));							
			}
		}
