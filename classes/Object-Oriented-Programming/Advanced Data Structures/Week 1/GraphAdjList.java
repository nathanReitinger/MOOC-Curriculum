...
public class GraphAdjList extends Graph {
	 ...
	 */		
	 public List<Integer> getDistance2(int v) {
		...
		List<Integer> one_hop_out = new ArrayList<Integer>();
		List<Integer> two_hop = new ArrayList<Integer>();

		one_hop_out = getNeighbors(v);
		//no need for a loop, just grab the values directly 
		//System.out.println(one_hop_out + "\t\t" + "neighbors of v");		
		
		//loop through each of the returend neighbors and get next
		for (int q = 0; q < one_hop_out.size(); q++) {						
			two_hop.addAll(getNeighbors(one_hop_out.get(q))); 
			//System.out.println(two_hop_1);
		}
