...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		//for a better explanation of a degree sequence
		// see http://www.sanfoundry.com/java-program-generate-graph-given-fixed-degree-sequence/									
					
		List<Integer> degree_sequence = new ArrayList<Integer>();				
		List<Integer> to_add = null;
		List<Integer> to_add_in = null;
		//used for debugging
		...
		//System.out.println("The number of vertices: " + graphFromFile.getNumVertices());
		
		//go through this list in to_add + add up sum + put it in another list in ascending order
		//another for edges going out
		//one list for edges coming in	
		for (int i = 0; i < vertices; i ++) {									
			to_add = getNeighbors(i);														
			to_add_in = getInNeighbors(i); 										
			int size = to_add.size();
			int in_size = to_add_in.size();
			//add edges together for total edges 
			degree_sequence.add(size + in_size); 								
		}
		//return reversed sorted list
		Collections.sort(degree_sequence,Collections.reverseOrder());			
